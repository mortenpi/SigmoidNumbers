...
function LinearAlgebra.dot(n::Integer, x::AbstractArray{T}, incx::Integer, y::AbstractArray{T}, incy::Integer) where T<:Posit
    (n <= 0 || incx <= 0) && return zero(T)
    (n * incx) > length(x) && throw(BoundsError(y, n * incx))
    (n * incy) > length(y) && throw(BoundsError(y, n * incy))
...
function LinearAlgebra.dot(n::Integer, x::AbstractArray{Complex{T}}, incx::Integer, y::AbstractArray{Complex{T}}, incy::Integer) where T<:Posit
    (n <= 0 || incx <= 0) && return zero(T)
    (n * incx) > length(x) && throw(BoundsError(y, n * incx))
    (n * incy) > length(y) && throw(BoundsError(y, n * incy))
...
function LinearAlgebra.dot(x::AbstractArray{<:PositOrComplex{N,ES}}, y::AbstractArray{<:PositOrComplex{N,ES}}) where {N,ES}
    if length(x) != length(y)
        throw(length(x) > length(y) ? BoundsError(x, length(y) + 1) : BoundsError(y, length(x) + 1))
    end
    naked_dot(x, y)
