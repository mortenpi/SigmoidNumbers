...
Base.randn(::Type{PType}, dims::Integer...) where {PType <: SmallFixed} = map(PType, randn(dims...))
...
for nindex = 16:-1:6
  for vindex = (nindex - 2):-1:2
    PType = SmallFixed{nindex, vindex}
    ...
    writedlm(string("./results/smallfixed-$nindex-$vindex.csv"), results, ',')
  end
end
